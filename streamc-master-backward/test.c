...
int main(int argc, char *argv[])
{
    ...
    printf("[TOTAL] Snum: %d Arrival: %f Repfreq: %f Pe: %f Seed: %d \n",snum,arrival,cp.repfreq,pe,fixrandseed);

    // seed PRNG
    struct timeval tv;
    gettimeofday(&tv, NULL);
    // unsigned int fixrandseed =0;
    srand(fixrandseed); // seed use microsec
    ...
    */
    ...
    if (correct) {
        ...
        printf("[Summary] snum: %d repfreq: %.3f erasure: %.3f nuses: %d T_P: %d\n", snum, cp.repfreq, pe, nuse, T_P);
    }
