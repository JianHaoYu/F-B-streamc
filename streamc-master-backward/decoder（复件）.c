...
static long A_div;    //矩阵A中进行的总的除法运算数量
static long A_add;    //矩阵A中进行的总的加法运算数量
    ...
    for (int i=dww-1; i>=0; i--) {
        if (aligned_coe[i] != 0) {
            if (dc->row[i] != NULL) {
                ...
                for(int k=0;k<i;k++){
                    if(aligned_coe[k] != 0 && aligned_row[i] != 0){ //需要相加的两个数均不为0才算作一次有限域内的加法
                       A_add+=1;
                    }
                }
                ...
                free(aligned_row);
                C_add+=256;  //一次galois_multiply_add_region操作大概率在矩阵C中进行256次有限域加法
...
int deactivate_decoder(struct decoder *dc)
{
    ...
    // DEBUG_PRINT(("A_add: %ld A_div: %ld\n",A_add,A_div));
    // DEBUG_PRINT(("C_add: %ld C_div: %ld\n",C_add,C_div));
    n_repair = 0;
    n_row_sub_ops = 0;
    n_row_ops=0;
    convert_diagonal_to_1=0;
